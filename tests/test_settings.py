...
class UcOrphanSweepDelayFloorTest(_SettingsTestBase):
    """Roadmap 6521bbce: a malformed KERDOOS_UC_ORPHAN_SWEEP_DELAY_SECONDS
    warns and floors to the default instead of crashing at settings-read
    time, and an unset one uses the uc tier's own default
    (uc.ORPHAN_SWEEP_DELAY_SECONDS)."""

    def test_unset_uses_default(self) -> None:
        settings = get_settings()
        self.assertEqual(
            settings.uc_orphan_sweep_delay_seconds,
            DEFAULT_UC_ORPHAN_SWEEP_DELAY_SECONDS)

    def test_valid_value_passes_through_unchanged(self) -> None:
        os.environ["KERDOOS_UC_ORPHAN_SWEEP_DELAY_SECONDS"] = "9"
        settings = get_settings()
        self.assertEqual(settings.uc_orphan_sweep_delay_seconds, 9.0)
    ...
    def test_invalid_or_non_positive_values_float_to_default_with_warning(
        ...
        for raw in ("abc", "0", "-1", ""):
            with self.subTest(raw=raw):
                os.environ["KERDOOS_UC_ORPHAN_SWEEP_DELAY_SECONDS"] = raw
                with self.assertLogs("kerdoos.config", level="WARNING") as cm:
                    settings = get_settings()
                self.assertEqual(
                    settings.uc_orphan_sweep_delay_seconds,
                    DEFAULT_UC_ORPHAN_SWEEP_DELAY_SECONDS)
                self.assertTrue(
                    any("KERDOOS_UC_ORPHAN_SWEEP_DELAY_SECONDS" in msg
                        for msg in cm.output), cm.output)
